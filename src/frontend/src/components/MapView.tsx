...
import {
  type Location as BackendLocation,
  LocationType,
  NoiseLevel,
  type Rating,
  WifiSpeed,
} from "../backend";
import type { NoiseFilter, WifiFilter } from "../constants/filters";
...
export function MapView({ userName }: MapViewProps) {
  ...
  const { mutateAsync: checkIn, isPending: _isCheckingIn } = useCheckIn();
  const { mutateAsync: updateRating, isPending: _isUpdating } =
    useUpdateRating();
